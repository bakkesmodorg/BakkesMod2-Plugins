...
void BallPredictionPlugin::onLoad()
{
	...
	gameWrapper->HookEvent("Function TAGame.GameEvent_Tutorial_TA.OnInit", bind(&BallPredictionPlugin::OnFreeplayLoad, this, std::placeholders::_1));
	gameWrapper->HookEvent("Function TAGame.GameEvent_Tutorial_TA.Destroyed", bind(&BallPredictionPlugin::OnFreeplayDestroy, this, std::placeholders::_1));
...
void BallPredictionPlugin::Render(CanvasWrapper canvas)
{
	if (*predictOn && gameWrapper->IsInGame())
	{
		...
		for (int i = 1; i < (*predictSteps); i++)
		{
			...
			if (currentBallLocation2D.X < 0 || currentBallLocation2D.X > 1920 || currentBallLocation2D.Y < 0 || currentBallLocation2D.Y > 1080)
				continue;
